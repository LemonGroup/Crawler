...
public class CrawlerDAOImpl<T> implements CrawlerDAO<T> {
    ...
    public void saveOrUpdate(T object) throws HibernateException {
        saveOrUpdate(Arrays.asList(object));
    }

    @Override
    public void saveOrUpdate(List<T> objects) throws HibernateException {
        Session session = SFactory.get().getCurrentSession();
        Transaction transaction = session.beginTransaction();
        int i = 0;
        int batchSize = SFactory.get().getSessionFactoryOptions().getJdbcBatchSize();
    ...
    @Override
    public void merge(T object) throws HibernateException {
        merge(Arrays.asList(object));
    }

    @Override
    public void merge(List<T> objects) throws HibernateException {
        Session session = SFactory.get().getCurrentSession();
        Transaction transaction = session.beginTransaction();
        int i = 0;
        int batchSize = SFactory.get().getSessionFactoryOptions().getJdbcBatchSize();
